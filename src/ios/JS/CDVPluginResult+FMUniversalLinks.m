//
//  CDVPluginResult+FMUniversalLinks.m
//
...
@implementation CDVPluginResult (FMUniversalLinks)
 ...
 */
+ (NSDictionary*)prepareMessageForHost:(FMULHost*)host originalURL:(NSURL*)originalURL {
 ...
 *  @return event name
 */
+ (NSString*)getEventNameBasedOnHost:(FMULHost*)host originalURLComponents:(NSURLComponents*)urlComponents {
	NSString* eventName = host.event;
	NSArray<FMULPath*>* hostPaths = host.paths;
	...
	for (FMULPath* hostPath in hostPaths) {
